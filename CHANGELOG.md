...
### Added
- **O&M Overview 오늘 예정작업**: 2주예측 정비계획 데이터 연동, Overview에 오늘 예정작업 박스 추가
...
### Fixed
- **터빈별 탭 날짜 변경**: 독립 데이터 로드 + 배지 업데이트 정상 동작
- **터빈별/2주예측 진입 조건**: hidden 클래스 체크로 변경 (offsetParent 오류 해결)
...
### Added — 하루전예측
- **72시간 예측 확장**: 24시간 → 72시간 (D+1~D+3) 예측 표시
- **풍속/발전량 차트 분리**: 복합 차트(Bar+Line) → 풍속 차트 + 발전량 차트 독립
- **실측값 표시**: 과거 날짜 선택 시 예측(점선) vs 실측(실선) 비교
  - 오늘 0시 기준까지만 실측 표시 (부분 실측 시 `Nh/72h` 표기)
- **예측 오차율 KPI**: 풍속 MAE (m/s), 발전량 nMAE (%) 자동 계산
- **3일 KPI**: 어제 실측 / 오늘 예상 / 내일 예상 (또는 D+1~D+3 예상)
- **터빈별 72시간**: 발전량 라인 차트 + 풍속 컬러 바 (실측/예측 2줄)
- **해상풍력 풍속 패턴**: 24시간 내내 6~12m/s 유지 (기존 PV 패턴 수정)

### Added — 2주예측
- **날짜 선택 기능**: `◀ YYYY-MM-DD (N일 전) ▶ [오늘]` 날짜 네비게이터
- **날짜 기반 라벨**: D+1~D+14 → 실제 날짜(M/D)로 표시 (박스플롯, 차트, 히트맵)
- **기상 박스플롯 체크박스 조합**: 풍속(고정) + 기온/파고 자유 선택 (4가지 조합)
  - Y축 범위 고정: 풍속 0~30, 기온 -10~40, 파고 0~3.5
  - 3축 동시 표시 지원 (풍속+기온+파고)
- **히트맵 모드 전환**: 일간 전체(0~24시) / O&M 작업시간(08~18시) 라디오 선택
- **히트맵 절대값 색상**: 날짜별 상대 비교 → 절대값 5등분 구간
  - 일간: 0~72 MWh (3MW × 24h), O&M: 0~30 MWh (3MW × 10h)
- **히트맵 셀 수치 표시**: 발전량(MWh) + 풍속(m/s) 직접 표시
- **14일 O&M 요약 날짜 연동**: 날짜 선택 시 요약 테이블 날짜도 변경

### Added — 정비계획 연동 (2주예측)
- **정비 입력 모달**: 히트맵 셀 클릭 → 정비 유형/시간/메모 입력 (localStorage 저장)
- **정비 삭제**: 기존 이력 있을 때 삭제 버튼 표시
- **히트맵 정비 마킹**: 입력된 셀에 빨간 테두리 + 🔧 배지
- **손실 발전량 표시**: 히트맵 손실 열, 발전소 KPI (빨간 카드)
- **정비 반영 발전량**: 발전소 KPI (초록 카드) + 차트 빨간 점선 라인
- **정비 내역 요약**: 일자/터빈/유형/시간/손실량 목록

### Changed
- **2주 기상 박스플롯**: 체크박스 3개(개별 차트) → 체크박스 조합(통합 차트)
- **발전량 생성 현실화**: 3MW 터빈 기준 풍속별 이용률 계산

...
### 📊 다중 시간대 예측
- **단기(하루전) 예측**: 72시간(D+1~D+3) 풍속/발전량 예측, 과거 실측 비교 및 오차율(MAE/nMAE)
...
- **O&M 상세 페이지**: 상단 바(← 전체, 발전소 드롭다운, Overview/하루전/2주/3개월 탭)
- **페이지 이동**: 홈 O&M 카드 클릭 → O&M 상세, ← 전체 → 홈 복귀
- **3개월 기상 박스플롯**: 체크박스 자유 조합(풍속+기온+파고), Y축 범위 고정
- **박스플롯 상세 패널**: 클릭 시 스크롤 위치 유지 (2주/3개월)

### v1.7.0 (2026-04-20)
- **하루전예측 72시간 확장**: 풍속/발전량 차트 분리, D+1~D+3 예측
- **하루전예측 실측 비교**: 과거 날짜 선택 시 예측 vs 실측 + 오차율(MAE/nMAE)

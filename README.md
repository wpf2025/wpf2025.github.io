...
### 📊 다중 시간대 예측
- **단기(3일) 예측**: 72시간(D+0~D+2) 풍속/발전량 예측, 과거 실측 비교 및 오차율(MAE/nMAE)
- **중기(2주) 예측**: 주간/일간 발전량 예측, 날짜별 예측 결과 관리
- **장기(3개월) 예측**: 월간/주간 발전량 및 기상 예측

### 🎯 예측 범위
- **발전소 전체 예측**: 통합 발전량 분석
- **터빈별 예측**: 개별 터빈(WTG #1~#20) 발전량 차트 + 풍속 컬러 바
- **정비 계획 연동**: 히트맵 셀 클릭으로 정비 입력, 손실 발전량 자동 계산
- **기상 예측 분석**: Box-and-whisker plot 기반 기상 변수 분포 분석 (풍속+기온+파고 동시 표시)
...
### 2. 단기 예측 (3일)
- **발전소 전체**: 24시간 풍속 예측 + 발전량 KPI/시간별 차트
- **터빈별 예측**: WTG#1~#20 발전량+풍속 듀얼 Y축 차트

### 3. 2주 예측
- **발전소 전체**: 기상 예측(체크박스 토글) + 발전량 예측
- **터빈별 예측**: 20호기 히트맵+랭킹 Overview, 상세 페이지
...
- **평가 기준**
  - 풍속: < 13m/s (양호), < 15m/s (주의), ≥ 15m/s (위험)
  - 최고기온: < 28°C (양호), < 30°C (주의), ≥ 30°C (위험)
  - 최저기온: > 8°C (양호), > 5°C (주의), ≤ 5°C (위험)
  - 강수량: < 7mm (양호), < 10mm (주의), ≥ 10mm (위험)
  - 파고: < 1.5m (양호), < 2m (주의), ≥ 2m (위험)
...
### 기상 변수별 케이스
**풍속:**
- 매우 약풍 (0-3 m/s) - 스카이블루: 발전 불가
- 약풍 (3-6 m/s) - 파란색: 저발전
- 중간풍 (6-10 m/s) - 녹색: 최적 발전
- 강풍 (10-15 m/s) - 주황색: 정격 출력
- 매우 강풍 (≥15 m/s) - 빨간색: 고출력/위험
...
### v1.9.2 (2026-06-05)
- **O&M Overview 차트 통합**: 풍속+발전량 두 개 차트를 듀얼 축 단일 차트로 합침
- **차트 스타일 변경**: 풍속 area fill(배경) + 발전량 line(전면) — 인과관계 직관적 표현
- **버그 수정**: Overview 차트 미표시 (가시성 체크 classList 기반으로 수정)

### v1.9.1 (2026-06-04)
- **탭 이름 변경**: "하루 전 예측" → "단기 예측" (3일 예측 의미 명확화)
- **단기예측 시작 날짜 변경**: D+1~D+3 → D+0~D+2 (오늘부터 72시간 예측)
- **O&M Overview 오늘 예정작업**: 2주예측 정비계획 데이터 연동하여 Overview에 오늘 예정작업 박스 추가
- **2주예측 시작 날짜 변경**: 기본 시작 날짜를 오늘 날짜 기준으로 변경
- **2주예측 캘린더 피커**: 날짜 라벨 클릭 시 캘린더 피커로 날짜 선택 가능
- **2주예측 실데이터 추가**: 2025-11-30, 2026-04-07, 2026-04-14, 2026-05-16~05-20 (서남해)
- **터빈별 탭 버그 수정**: 날짜 변경 시 독립 데이터 로드 + 배지 업데이트, 진입 조건 hidden 클래스 체크로 변경
- **터빈별 히트맵**: 손실 발전량 소수점 1자리 표시

### v1.9.0 (2026-05-15)
- **2주예측 실데이터 연동**: JSON 파일 기반 데이터 로드 (fetch 성공 → 실데이터, 404 → 랜덤 fallback)
- **데이터 형식**: `data/2week/SNH/{날짜}_plant.json` + `{날짜}_turbines.json`
- **CSV→JSON 변환**: DA14_NWP/Power/Turbine CSV에서 자동 변환
- **기상 O&M 시간대 선택**: 일간 전체(0~24시) / O&M 시간(08~18시) 라디오 선택
- **기온 박스플롯 색상**: ≤5°C Blue, 5~30°C Green, ≥30°C Red
- **정비 손실 계산**: 실제 터빈별 시간대 발전량 기반
- **데이터 상태 배지**: "예측 데이터"(파랑) / "SAMPLE"(빨강)

### v1.8.0 (2026-04-29)
- **메뉴 구조 재편**: 사이드바를 WPF 시스템 전체 메뉴로 변경
- **메인 홈 대시보드**: 서버 자원, 모델 학습/추론, O&M 발전소 현황(3개소), VPP 예측 현황
- **O&M 상세 페이지**: 상단 바(← 전체, 발전소 드롭다운, Overview/단기/2주/3개월 탭)
- **페이지 이동**: 홈 O&M 카드 클릭 → O&M 상세, ← 전체 → 홈 복귀
- **3개월 기상 박스플롯**: 체크박스 자유 조합(풍속+기온+파고), Y축 범위 고정
- **박스플롯 상세 패널**: 클릭 시 스크롤 위치 유지 (2주/3개월)

### v1.7.0 (2026-04-20)
- **단기예측 72시간 확장**: 풍속/발전량 차트 분리, D+1~D+3 예측
- **단기예측 실측 비교**: 과거 날짜 선택 시 예측 vs 실측 + 오차율(MAE/nMAE)
- **터빈별 풍속 컬러 바**: 72시간 풍속을 구간별 색상 바로 표시
- **2주예측 날짜 선택**: 날짜별 예측 결과 조회, 실제 날짜 라벨
- **2주예측 기상 박스플롯**: 풍속+기온+파고 자유 조합 체크박스, Y축 범위 고정
- **히트맵 모드 전환**: 일간 전체(0~24시) / O&M 작업시간(08~18시)
- **히트맵 절대값 색상**: 3MW 터빈 기준 5등분 (일간 0~72, O&M 0~30 MWh)
- **정비계획 연동**: 히트맵 셀 클릭 정비 입력/삭제, 손실 발전량 KPI/차트 반영

### v1.5.0 (2026-03-18)
- **메인페이지 재설계**: 풍속/발전량 차트 + Leaflet 위성지도(WTG#1~#20 실좌표 마커)
- **하루전예측 신규**: 24시간 풍속 라인 + 발전량 차트, 20호기 듀얼 Y축 차트
- **탭 이름 변경**: 단기→하루전, 중기→2주, 장기→3개월, 개요→서남해해상풍력
- **TBD 탭 정리**: 풍속예측/정비스케줄 HTML/JS 코드 완전 제거
- **SAMPLE 배지 전체 제거**
- **버그 수정**: Leaflet 중복 초기화, querySelector 숫자 ID 에러

### v1.4.0 (2026-03-17)
- **장기예측 모달 확장**: 박스플롯 클릭 시 상세 모달 개선
  - 주간 일별 상세 기상 테이블 추가 → O&M 가능 여부 판단
  - 종합 상태, 신뢰도, 풍속/기온/파고 원형 인디케이터 + 수치 표시
  - 각 변수별 평가 기준 표시 (≤ 10m/s, ≤ 30°C, ≥ 5°C, ≤ 1.5m)
  - 세부 수치는 마우스 Tooltip으로 표시
  - 모달 타이틀에 해당 주차 날짜 범위 표시
  - 모달 크기 확대 (max-w-4xl → max-w-6xl)
- **중기예측 기상 예측 추가**: 14일 박스플롯 + 24시간 O&M 모달
  - 풍속/기온/파고 14일 박스플롯 (별도 카드)
  - 일자 클릭 시 24시간 O&M 가능 여부 판단 모달
  - 시간별 풍속 예측 라인 차트 (단기예측 스타일)
  - O&M 기준: 풍속 ≤ 10m/s, 기온 ≤ 30°C / ≥ 5°C, 파고 ≤ 1.5m
- **중기예측 레이아웃 개선**:
  - 주평균(1/4) + 일평균(3/4) 한 줄 레이아웃
  - 주평균 바 차트 제거, KPI 카드만 표시
...
- **도넛 차트 UI 개선**:
  - Chart.js 내장 범례 → HTML 범례로 대체하여 도넛 크기 동일하게 통일
  - Y축 스케일 제거 (도넛 차트에 불필요한 scales 비활성화)
...
### v1.3.4 (2025-10-24)
- **코드 리팩토링**: 유지보수성 및 일관성 향상
  - 기상 기준값 상수화 (`WEATHER_THRESHOLDS`, `STATUS_COLORS`)
  - 날씨 데이터 생성 함수 모듈화
  - 중복 코드 제거 및 함수 통합
- **기능 개선**:
  - 90일 예측 차트 모드 파고 기준 2m 적용
  - 모든 예측 모드에서 동일한 데이터 생성 로직 사용
- **코드 품질**:
  - 사용하지 않는 `getWeatherStatus()` 함수 제거
  - 하드코딩된 값 제거로 기준값 변경 용이
  - 일관된 상태 색상 관리
